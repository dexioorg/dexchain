...
use frame_support::{
    decl_error, decl_event, decl_module, decl_storage, dispatch::DispatchResultWithPostInfo, log,
        ...
        fn on_initialize(n: T::BlockNumber) -> Weight {
            ...
            if let Ok(log) = fp_consensus::find_pre_log(&frame_system::Module::<T>::digest()) {
                ...
                for transaction in block.transactions {
					...
					.expect("pre-block recover_signer verification failed; the block cannot be built");
...
impl<T: Config> Module<T> {
    ...
    pub fn do_transact(source: H160, transaction: ethereum::Transaction) -> DispatchResultWithPostInfo {
